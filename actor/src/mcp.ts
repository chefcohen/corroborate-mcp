...
export async function getMcpProxyClient(command: string[]): Promise<Client> {
    ...
    // env must be passed EXPLICITLY: the SDK's default child environment is sanitized, and the
    // hosted tier's engine restriction (legal floor) rides on CORROBORATE_ENGINES reaching the child.
    const transport = new StdioClientTransport({
        command: command[0],
        args: command.slice(1),
        env: {
            ...getDefaultEnvironment(),
            CORROBORATE_ENGINES: process.env.CORROBORATE_ENGINES ?? 'gdelt,hackernews',
        },
    });
