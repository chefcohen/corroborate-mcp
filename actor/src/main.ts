...
// Command to run the Corroborate MCP server (installed from npm as a dependency).
// LEGAL FLOOR: the hosted tier runs license-clean engines ONLY — CORROBORATE_ENGINES is passed
// to the child explicitly in mcp.ts (the MCP SDK's default child environment is sanitized and
// would silently drop it otherwise).
const MCP_COMMAND = ['npx', 'corroborate-mcp'];
